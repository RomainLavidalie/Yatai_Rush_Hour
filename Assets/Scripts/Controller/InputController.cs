...
public class InputController : Controller
{
    ...
    void Update()
    {
        ...
        {
            Vector3 WantedDirectionLookRightTargetSmooth =
                Vector3.Cross(transform.up, WantedDirectionLookTargetSmooth).normalized;
            Quaternion rotateHorizontal = Quaternion.AngleAxis(axisLook.x * XSensivity * Time.deltaTime, Vector3.up);
            Quaternion rotateVertical = Quaternion.AngleAxis(-axisLook.y * YSensivity * Time.deltaTime,
                WantedDirectionLookRightTargetSmooth);
            ...
            if (angleWithUp < angleMinWithUp)
                rotateVertical *=
                    Quaternion.AngleAxis(angleMinWithUp - angleWithUp, WantedDirectionLookRightTargetSmooth);
            float angleMaxWithUp = 90 + MaxUpDownAngle;
            if (angleWithUp > angleMaxWithUp)
                rotateVertical *=
                    Quaternion.AngleAxis(angleMaxWithUp - angleWithUp, WantedDirectionLookRightTargetSmooth);
